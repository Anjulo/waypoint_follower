...
class WaypointFollower():
    ...
    def steer_control(self, error_y, error_yaw, integral_prior_y,integral_prior_yaw, wpts):
        ...
        # P controller 1,0.3
        """
        kp_y   = 0.45 # P gain w.r.t. cross track error
        kp_yaw = 0.14 # P gain w.r.t. yaw error
        ...
        steer = kp_y*error_y + kp_yaw*error_yaw + ki_y*integral_y + ki_yaw*integral_yaw
        ...
        steer = np.arctan((2**np.sin(alpha))/ld)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        # transforming the origin to front axle for stanley control
        
        L=0.207       # wheel-base length
        fa_x = ego_x + L*np.cos(ego_yaw)
        fa_y = ego_y + L*np.sin(ego_yaw)
